...
class EpicJungle_Wizard {
	...
	/**
	 * The base path where EpicJungle_Wizard is located.
	 *
	 * @var array $strings
	 */
	...
	/**
	 * The base url where EpicJungle_Wizard is located.
	 *
	 * @var array $strings
	 */
	...
	/**
	 * The location where EpicJungle_Wizard is located within the theme or plugin.
	 *
	 ...
	 *
	 * @var string $epicjungle_wizard_url
	 */
	protected $epicjungle_wizard_url = null;
	...
	private function version() {
		...
		if ( ! defined( 'EPICJUNGLE_WIZARD_VERSION' ) ) {
			define( 'EPICJUNGLE_WIZARD_VERSION', '1.0.0' );
		}
	}
	...
	function __construct( $config = array(), $strings = array() ) {
		...
		$config = wp_parse_args(
			$config, array(
				...
				'directory' => 'epicjungle_wizard',
				'epicjungle_wizard_url' => 'epicjungle_wizard',
		...
		$this->epicjungle_wizard_url = $config['epicjungle_wizard_url'];
		...
		if ( true !== $this->dev_mode ) {
			...
			$already_setup = get_option( 'epicjungle_wizard_' . $this->slug . '_completed' );

			// Return if EpicJungle_Wizard has already completed it's setup.
			if ( $already_setup ) {
				return;
			}
		}
		...
		require_once trailingslashit( $this->base_path ) . 'inc/' . $this->directory . '/includes/class-epicjungle-wizard-logger.php';
	
		$this->logger = EpicJungle_Wizard_Logger::get_instance();
		...
		add_action( 'wp_ajax_epicjungle_wizard_content', array( $this, '_ajax_content' ), 10, 0 );
		add_action( 'wp_ajax_epicjungle_wizard_get_total_content_import_items', array( $this, '_ajax_get_total_content_import_items' ), 10, 0 );
		add_action( 'wp_ajax_epicjungle_wizard_plugins', array( $this, '_ajax_plugins' ), 10, 0 );
		add_action( 'wp_ajax_epicjungle_wizard_child_theme', array( $this, 'generate_child' ), 10, 0 );
		add_action( 'wp_ajax_epicjungle_wizard_activate_license', array( $this, '_ajax_activate_license' ), 10, 0 );
		add_action( 'wp_ajax_epicjungle_wizard_update_selected_import_data_info', array( $this, 'update_selected_import_data_info' ), 10, 0 );
		add_action( 'wp_ajax_epicjungle_wizard_import_finished', array( $this, 'import_finished' ), 10, 0 );
	...
	function required_classes() {
		...
		require_once trailingslashit( $this->base_path ) . 'inc/' . $this->directory . '/includes/class-epicjungle-wizard-downloader.php';

		$this->importer = new ProteusThemes\WPContentImporter2\Importer( array( 'fetch_attachments' => true ), $this->logger );

		require_once trailingslashit( $this->base_path ) . 'inc/' . $this->directory . '/includes/class-epicjungle-wizard-widget-importer.php';

		if ( ! class_exists( 'WP_Customize_Setting' ) ) {
			require_once ABSPATH . 'wp-includes/class-wp-customize-setting.php';
		}

		require_once trailingslashit( $this->base_path ) . 'inc/' . $this->directory . '/includes/class-epicjungle-wizard-customizer-option.php';
		require_once trailingslashit( $this->base_path ) . 'inc/' . $this->directory . '/includes/class-epicjungle-wizard-customizer-importer.php';
		require_once trailingslashit( $this->base_path ) . 'inc/' . $this->directory . '/includes/class-epicjungle-wizard-hooks.php';

		$this->hooks = new EpicJungle_Wizard_Hooks();
	...
	public function switch_theme() {
		if ( ! is_child_theme() ) {
			set_transient( $this->theme->template . '_epicjungle_wizard_redirect', 1 );
		}
	}
	...
	public function redirect() {
		...
		if ( ! get_transient( $this->theme->template . '_epicjungle_wizard_redirect' ) ) {
			return;
		}

		delete_transient( $this->theme->template . '_epicjungle_wizard_redirect' );

		wp_safe_redirect( menu_page_url( $this->epicjungle_wizard_url ) );

		exit;
	}

	/**
	 * Give the user the ability to ignore EpicJungle_Wizard WP.
	 */
	public function ignore() {

		// Bail out if not on correct page.
		if ( ! isset( $_GET['_wpnonce'] ) || ( ! wp_verify_nonce( $_GET['_wpnonce'], 'epicjungle_wizardwp-ignore-nounce' ) || ! is_admin() || ! isset( $_GET[ $this->ignore ] ) || ! current_user_can( 'manage_options' ) ) ) {
			return;
		}

		update_option( 'epicjungle_wizard_' . $this->slug . '_completed', 'ignored' );
	}
	...
	public function add_admin_menu() {
		...
		$this->hook_suffix = add_submenu_page(
			esc_html( $this->parent_slug ), esc_html( $strings['admin-menu'] ), esc_html( $strings['admin-menu'] ), sanitize_key( $this->capability ), sanitize_key( $this->epicjungle_wizard_url ), array( $this, 'admin_page' )
		);
	}
	...
	public function admin_page() {
		...
		if ( empty( $_GET['page'] ) || $this->epicjungle_wizard_url !== $_GET['page'] ) {
			return;
		}
		...
		// Enqueue styles.
		wp_enqueue_style( 'epicjungle_wizard', trailingslashit( $this->base_url ) . 'inc/' . $this->directory . '/assets/css/epicjungle-wizard' . $suffix . '.css', array( 'wp-admin' ), EPICJUNGLE_WIZARD_VERSION );

		// Enqueue javascript.
		wp_enqueue_script( 'epicjungle_wizard', trailingslashit( $this->base_url ) . 'inc/' . $this->directory . '/assets/js/epicjungle-wizard' . $suffix . '.js', array( 'jquery-core' ), EPICJUNGLE_WIZARD_VERSION );

		$texts = array(
			'something_went_wrong' => esc_html__( 'Algo deu errado. Atualize a página e tente novamente!', 'epicjungle' ),
		);
		...
		if ( class_exists( 'TGM_Plugin_Activation' ) ) {
			...
			wp_localize_script(
				'epicjungle_wizard', 'epicjungle_wizard_params', array(
					...
					'wpnonce'          => wp_create_nonce( 'epicjungle_wizard_nonce' ),
					'texts'            => $texts,
				)
			);
		} else {
		...
		<div class="epicjungle_wizard__wrapper">

			<div class="epicjungle_wizard__content epicjungle_wizard__content--<?php echo esc_attr( strtolower( $this->steps[ $this->step ]['name'] ) ); ?>">
			...
			<?php $ignore_url = wp_nonce_url( admin_url( '?' . $this->ignore . '=true' ), 'epicjungle_wizardwp-ignore-nounce' ); ?>
	...
	protected function header() {
		...
		<body class="epicjungle_wizard__body epicjungle_wizard__body--<?php echo esc_attr( $current_step ); ?>">
		<?php
	}
	...
	public function svg_sprite() {
		...
		$svg = trailingslashit( $this->base_path ) . 'inc/' . $this->directory . '/assets/images/sprite.svg';

		// If it exists, include it.
		if ( file_exists( $svg ) ) {
			require_once apply_filters( 'epicjungle_wizard_svg_sprite', $svg );
		}
	}
	...
	public function svg( $args = array() ) {
		...
		if ( empty( $args ) ) {
			return __( 'Por favor, defina os parâmetros padrão na forma de uma matriz.', 'epicjungle' );
		}

		// Define an icon.
		if ( false === array_key_exists( 'icon', $args ) ) {
			return __( 'Por favor, defina um nome de arquivo de ícone SVG.', 'epicjungle' );
		}
	...
	public function svg_allowed_html() {
		...
		return apply_filters( 'epicjungle_wizard_svg_allowed_html', $array );
	}

	/**
	 * Loading epicjungle_wizard-spinner.
	 */
	public function loading_spinner() {

		// Define the spinner file.
	//	$spinner = $this->directory . '/assets/images/spinner.php';
		$spinner = get_template_directory() . '/inc/epicjungle_wizard/assets/images/spinner.php';

		// Retrieve the spinner.
		get_template_part( apply_filters( 'epicjungle_wizard_loading_spinner', $spinner ) );
	}
	...
	public function loading_spinner_allowed_html() {
		...
		return apply_filters( 'epicjungle_wizard_loading_spinner_allowed_html', $array );
	}
	...
	public function steps() {
		...
		$this->steps = array(
			'welcome' => array(
				'name'    => esc_html__( 'Bem vindo', 'epicjungle' ),
		...
		if ( class_exists( 'TGM_Plugin_Activation' ) ) {
			$this->steps['plugins'] = array(
				'name' => esc_html__( 'Plugins', 'epicjungle' ),
		...
		if ( ! empty( $this->import_files ) ) {
			$this->steps['content'] = array(
				'name' => esc_html__( 'Conteúdo', 'epicjungle' ),
				'view' => array( $this, 'content' ),
			);
		}

		$this->steps['ready'] = array(
			'name' => esc_html__( 'Pronto', 'epicjungle' ),
			'view' => array( $this, 'ready' ),
		);

		$this->steps = apply_filters( $this->theme->template . '_epicjungle_wizard_steps', $this->steps );
	}
	...
	protected function welcome() {
		...
		<div class="epicjungle_wizard__content--transition">
		...
		<footer class="epicjungle_wizard__content__footer">
			<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '/' ) ); ?>" class="epicjungle_wizard__button epicjungle_wizard__button--skip"><?php echo esc_html( $no ); ?></a>
			<a href="<?php echo esc_url( $this->step_next_link() ); ?>" class="epicjungle_wizard__button epicjungle_wizard__button--next epicjungle_wizard__button--proceed epicjungle_wizard__button--colorchange"><?php echo esc_html( $start ); ?></a>
			<?php wp_nonce_field( 'epicjungle_wizard' ); ?>
		</footer>

	<?php
		$this->logger->debug( __( 'A etapa de boas-vindas foi exibida', 'epicjungle' ) );
	}
	...
	protected function welcome_handler() {
		...
		check_admin_referer( 'epicjungle_wizard' );
	...
	protected function license() {
		...
		<div class="epicjungle_wizard__content--transition">
			...
			<?php if ( ! $is_theme_registered ) : ?>
				<div class="epicjungle_wizard__content--license-key">
					<label for="license-key"><?php echo esc_html( $label ); ?></label>

					<div class="epicjungle_wizard__content--license-key-wrapper">
		...
		<footer class="epicjungle_wizard__content__footer <?php echo esc_attr( $is_theme_registered_class ); ?>">

			<?php if ( ! $is_theme_registered ) : ?>

				<?php if ( ! $required ) : ?>
					<a href="<?php echo esc_url( $this->step_next_link() ); ?>" class="epicjungle_wizard__button epicjungle_wizard__button--skip epicjungle_wizard__button--proceed"><?php echo esc_html( $skip ); ?></a>
				<?php endif ?>

				<a href="<?php echo esc_url( $this->step_next_link() ); ?>" class="epicjungle_wizard__button epicjungle_wizard__button--next button-next js-epicjungle_wizard-license-activate-button" data-callback="activate_license">
					<span class="epicjungle_wizard__button--loading__text"><?php echo esc_html( $install ); ?></span>
					<?php echo wp_kses( $this->loading_spinner(), $this->loading_spinner_allowed_html() ); ?>
				</a>

			<?php else : ?>
				<a href="<?php echo esc_url( $this->step_next_link() ); ?>" class="epicjungle_wizard__button epicjungle_wizard__button--next epicjungle_wizard__button--proceed epicjungle_wizard__button--colorchange"><?php echo esc_html( $next ); ?></a>
			<?php endif; ?>
			<?php wp_nonce_field( 'epicjungle_wizard' ); ?>
		</footer>
		<?php
		$this->logger->debug( __( 'A etapa de ativação da licença foi exibida', 'epicjungle' ) );
	}
	...
	private function is_theme_registered() {
		...
		return apply_filters( 'epicjungle_wizard_is_theme_registered', $is_registered );
	}
	...
	protected function child() {
		...
		$child_theme_option = get_option( 'epicjungle_wizard_' . $this->slug . '_child' );
		...
		<div class="epicjungle_wizard__content--transition">
			...
			<a class="epicjungle_wizard__button epicjungle_wizard__button--knockout epicjungle_wizard__button--no-chevron epicjungle_wizard__button--external" href="<?php echo esc_url( $action_url ); ?>" target="_blank"><?php echo esc_html( $action ); ?></a>
		...
		<footer class="epicjungle_wizard__content__footer">
			...
			<?php if ( ! $is_child_theme ) : ?>
				...
				<a href="<?php echo esc_url( $this->step_next_link() ); ?>" class="epicjungle_wizard__button epicjungle_wizard__button--next button-next" data-callback="install_child">
					<span class="epicjungle_wizard__button--loading__text"><?php echo esc_html( $install ); ?></span>
					<?php echo wp_kses( $this->loading_spinner(), $this->loading_spinner_allowed_html() ); ?>
				</a>
	...
	<?php
		$this->logger->debug( __( 'A etapa de instalação do tema filho foi exibida', 'epicjungle' ) );
	}
	...
	protected function plugins() {
		...
		// Variables.
		$url    = wp_nonce_url( add_query_arg( array( 'plugins' => 'go' ) ), 'epicjungle_wizard' );
		$method = '';
		...
		<div class="epicjungle_wizard__content--transition">
			...
			<?php if ( $count ) { ?>
				<a id="epicjungle_wizard__drawer-trigger" class="epicjungle_wizard__button epicjungle_wizard__button--knockout"><span><?php echo esc_html( $action ); ?></span><span class="chevron"></span></a>
			<?php } ?>
		...
		<form action="" method="post">
			...
			<?php if ( $count ) : ?>
				...
				<ul class="epicjungle_wizard__drawer epicjungle_wizard__drawer--install-plugins">
					...
					<?php foreach ( $required_plugins as $slug => $plugin ) : ?>
						<li data-slug="<?php echo esc_attr( $slug ); ?>">
							...
							<label for="default_plugins_<?php echo esc_attr( $slug ); ?>">
								...
								<span class="badge">
									<span class="hint--top" aria-label="<?php esc_html_e( 'Obrigatório', 'epicjungle' ); ?>">
										<?php esc_html_e( 'req', 'epicjungle' ); ?>
									</span>
								</span>
							</label>
						</li>
			...
			<footer class="epicjungle_wizard__content__footer <?php echo esc_attr( $class ); ?>">
				<?php if ( $count ) : ?>
					<a id="close" href="<?php echo esc_url( $this->step_next_link() ); ?>" class="epicjungle_wizard__button epicjungle_wizard__button--skip epicjungle_wizard__button--closer epicjungle_wizard__button--proceed"><?php echo esc_html( $skip ); ?></a>
					<a id="skip" href="<?php echo esc_url( $this->step_next_link() ); ?>" class="epicjungle_wizard__button epicjungle_wizard__button--skip epicjungle_wizard__button--proceed"><?php echo esc_html( $skip ); ?></a>
					<a href="<?php echo esc_url( $this->step_next_link() ); ?>" class="epicjungle_wizard__button epicjungle_wizard__button--next button-next" data-callback="install_plugins">
						<span class="epicjungle_wizard__button--loading__text"><?php echo esc_html( $install ); ?></span>
						<?php echo wp_kses( $this->loading_spinner(), $this->loading_spinner_allowed_html() ); ?>
					</a>
				<?php else : ?>
	...
	<?php
		$this->logger->debug( __( 'A etapa de instalação do plugin foi exibida', 'epicjungle' ) );
	}
	...
	protected function content() {
		...
		<div class="epicjungle_wizard__content--transition">
			...
			<?php if ( 1 < count( $this->import_files ) ) : ?>
				...
				<div class="epicjungle_wizard__select-control-wrapper">

					<select class="epicjungle_wizard__select-control js-epicjungle_wizard-demo-import-select">
						<?php foreach ( $this->import_files as $index => $import_file ) : ?>
							<option value="<?php echo esc_attr( $index ); ?>"><?php echo esc_html( $import_file['import_file_name'] ); ?></option>
						<?php endforeach; ?>
					</select>

					<div class="epicjungle_wizard__select-control-help">
						<span class="hint--top" aria-label="<?php echo esc_attr__( 'Selecionar demonstrativo', 'epicjungle' ); ?>">
		...
		<form action="" method="post" class="<?php echo esc_attr( $multi_import ); ?>">
			...
			<ul class="epicjungle_wizard__drawer epicjungle_wizard__drawer--import-content js-epicjungle_wizard-drawer-import-content">
			...
			<footer class="epicjungle_wizard__content__footer">
				...
				<a href="<?php echo esc_url( $this->step_next_link() ); ?>" class="epicjungle_wizard__button epicjungle_wizard__button--next button-next" data-callback="install_content">
					<span class="epicjungle_wizard__button--loading__text"><?php echo esc_html( $import ); ?></span>

					<div class="epicjungle_wizard__progress-bar">
						<span class="js-epicjungle_wizard-progress-bar"></span>
					</div>

					<span class="js-epicjungle_wizard-progress-bar-percentage">0%</span>
				</a>
	...
	<?php
		$this->logger->debug( __( 'A etapa de importação de conteúdo foi exibida', 'epicjungle' ) );
	}
	...
	protected function ready() {
		...
		$links_class = empty( $links ) ? 'epicjungle_wizard__content__footer--nolinks' : null;
		...
		update_option( 'epicjungle_wizard_' . $this->slug . '_completed', time() );
		?>
		...
		<footer class="epicjungle_wizard__content__footer epicjungle_wizard__content__footer--fullwidth <?php echo esc_attr( $links_class ); ?>">

			<a href="<?php echo esc_url( $this->ready_big_button_url ); ?>" class="epicjungle_wizard__button epicjungle_wizard__button--blue epicjungle_wizard__button--fullwidth epicjungle_wizard__button--popin"><?php echo esc_html( $big_btn ); ?></a>

			<?php if ( ! empty( $links ) ) : ?>
				<a id="epicjungle_wizard__drawer-trigger" class="epicjungle_wizard__button epicjungle_wizard__button--knockout"><span><?php echo esc_html( $action ); ?></span><span class="chevron"></span></a>

				<ul class="epicjungle_wizard__drawer epicjungle_wizard__drawer--extras">
	...
	<?php
		$this->logger->debug( __( 'A etapa final foi exibida', 'epicjungle' ) );
	}
	...
	public function generate_child() {
		...
		if ( ! file_exists( $path ) ) {
			...
			if ( $this->theme->template !== $slug ) :
				update_option( 'epicjungle_wizard_' . $this->slug . '_child', $name );
				switch_theme( $slug );
			endif;

			$this->logger->debug( __( 'O tema filho existente foi ativado', 'epicjungle' ) );
		...
		$this->logger->debug( __( 'O tema filho recém-gerado foi ativado', 'epicjungle' ) );
	...
	public function _ajax_activate_license() {
		...
		if ( ! check_ajax_referer( 'epicjungle_wizard_nonce', 'wpnonce' ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Ops! A ativação do tema falhou. Tente novamente ou entre em contato com o suporte.', 'epicjungle' ),
				)
			);
		}
		...
		if ( empty( $_POST['license_key'] ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Adicione sua chave de licença antes de tentar ativar uma.', 'epicjungle' ),
				)
			);
		}
		...
		if ( ! has_filter( 'epicjungle_wizard_ajax_activate_license' ) ) {
			$result = $this->edd_activate_license( $license_key );
		} else {
			$result = apply_filters( 'epicjungle_wizard_ajax_activate_license', $license_key );
		}

		$this->logger->debug( __( 'A ativação da licença foi realizada com os seguintes resultados', 'epicjungle' ), $result );
	...
	protected function edd_activate_license( $license ) {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				$message = esc_html__( 'Ocorreu um erro. Por favor, tente novamente.', 'epicjungle' );
			}
		} else {
				...
				switch ( $license_data->error ) {
					...
					case 'expired':
						$message = sprintf(
							...
							esc_html__( 'Sua chave de licença expirou em %s.', 'epicjungle' ),
							date_i18n( get_option( 'date_format' ), strtotime( $license_data->expires, current_time( 'timestamp' ) ) )
						);
						break;

					case 'revoked':
						$message = esc_html__( 'Sua chave de licença foi desativada.', 'epicjungle' );
						break;

					case 'missing':
						$message = esc_html__( 'Esta parece ser uma chave de licença inválida. Tente novamente ou entre em contato com o suporte.', 'epicjungle' );
						break;

					case 'invalid':
					case 'site_inactive':
						$message = esc_html__( 'Sua licença não está ativa para este URL.', 'epicjungle' );
						break;

					case 'item_name_mismatch':
						/* translators: EDD Item Name */
						$message = sprintf( esc_html__( 'Esta parece ser uma chave de licença inválida para %s.', 'epicjungle' ), $this->edd_item_name );
						break;

					case 'no_activations_left':
						$message = esc_html__( 'Sua chave de licença atingiu o limite de ativação.', 'epicjungle' );
						break;
	...
	public function generate_child_functions_php( $slug ) {
		...
		$output = "
			...
			/*
			 * Se o seu tema filho tiver mais de um arquivo .css (por exemplo, ie.css, style.css, main.css), então
			 * você terá que certificar-se de manter todas as dependências do tema pai.
			 *
			 * Certifique-se de estar usando o identificador correto para carregar os estilos do tema pai.
			 * A falha em usar a tag apropriada resultará em um arquivo CSS sendo carregado desnecessariamente duas vezes.
			 * Isso geralmente não afetará a aparência do site, mas é ineficiente e prolonga o tempo de carregamento da sua página.
			 *
		...
		$this->logger->debug( __( 'O conteúdo functions.php do tema filho foi gerado', 'epicjungle' ) );

		// Filterable return.
		return apply_filters( 'epicjungle_wizard_generate_child_functions_php', $output, $slug );
	}
	...
	public function generate_child_style_css( $slug, $parent, $author, $version ) {
		...
		$output = "
			...
			* Description: Este é um tema filho do {$parent}.
			* Author: {$author}
			* Template: {$slug}
			* Version: {$version}
			*/\n
		...
		$this->logger->debug( __( 'O conteúdo style.css do tema filho foi gerado', 'epicjungle' ) );

		return apply_filters( 'epicjungle_wizard_generate_child_style_css', $output, $slug, $parent, $version );
	}
	...
	public function generate_child_screenshot( $path ) {
		...
		$screenshot = apply_filters( 'epicjungle_wizard_generate_child_screenshot', '' );
		...
		if ( ! empty( $screenshot ) && file_exists( $screenshot ) ) {
			...
			$this->logger->debug( __( 'A captura de tela do tema filho foi copiada para o tema filho, com o seguinte resultado', 'epicjungle' ), array( 'copied' => $copied ) );
		} else {
			$this->logger->debug( __( 'A captura de tela do tema filho não foi gerada devido a esses resultados', 'epicjungle' ), array( 'screenshot' => $screenshot ) );
		}
	}
	...
	function _ajax_plugins() {
		...
		if ( ! check_ajax_referer( 'epicjungle_wizard_nonce', 'wpnonce' ) || empty( $_POST['slug'] ) ) {
			exit( 0 );
		}
		...
		foreach ( $plugins['activate'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Ativando', 'epicjungle' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['update'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Atualizando', 'epicjungle' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['install'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Instalando', 'epicjungle' ),
				);
				break;
			}
		}
		...
		if ( $json ) {
			$this->logger->debug(
				__( 'Um plugin com os seguintes dados será processado', 'epicjungle' ),
				array(
			...
			$json['message'] = esc_html__( 'Instalando', 'epicjungle' );
			wp_send_json( $json );
		} else {
			$this->logger->debug(
				__( 'Um plugin com os seguintes dados foi processado', 'epicjungle' ),
				array(
					'plugin_slug' => $_POST['slug'],
				)
			);
			...
			wp_send_json(
				array(
					'done'    => 1,
					'message' => esc_html__( 'Sucesso', 'epicjungle' ),
				)
			);
		}
	...
	function _ajax_content() {
		...
		if ( ! check_ajax_referer( 'epicjungle_wizard_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
			$this->logger->error( __( 'A chamada AJAX do importador de conteúdo falhou ao iniciar devido a dados incorretos', 'epicjungle' ) );

			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Conteúdo inválido!', 'epicjungle' ),
				)
			);
		}
		...
		if ( isset( $_POST['proceed'] ) ) {
			if ( is_callable( $this_content['install_callback'] ) ) {
				$this->logger->info(
					__( 'A chamada AJAX de importação de conteúdo será executada com esses dados de importação', 'epicjungle' ),
					array(
			...
			$json = array(
				...
				'action'         => 'epicjungle_wizard_content',
				'proceed'        => 'true',
				'content'        => $_POST['content'],
				'_wpnonce'       => wp_create_nonce( 'epicjungle_wizard_nonce' ),
		...
		if ( $json ) {
			...
			$this->logger->error(
				__( 'A chamada AJAX de importação de conteúdo falhou com esses dados passados', 'epicjungle' ),
				array(
			...
			wp_send_json(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Erro', 'epicjungle' ),
					'logs'    => '',
					'errors'  => '',
				)
			);
	...
	public function _ajax_get_total_content_import_items() {
		if ( ! check_ajax_referer( 'epicjungle_wizard_nonce', 'wpnonce' ) && empty( $_POST['selected_index'] ) ) {
			$this->logger->error( __( 'A chamada AJAX do importador de conteúdo para recuperar o total de itens de importação de conteúdo falhou ao iniciar devido a dados incorretos.', 'epicjungle' ) );

			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Dados inválidos!', 'epicjungle' ),
				)
			);
		}
	...
	public function get_import_data_info( $selected_import_index = 0 ) {
		...
		if ( false !== has_action( 'epicjungle_wizard_after_all_import' ) ) {
	...
	protected function get_import_data( $selected_import_index = 0 ) {
		...
		if ( ! empty( $import_files['content'] ) ) {
			$content['content'] = array(
				'title'            => esc_html__( 'Conteúdo', 'epicjungle' ),
				'description'      => esc_html__( 'Dados de conteúdo de demonstração.', 'epicjungle' ),
				'pending'          => esc_html__( 'Pendente', 'epicjungle' ),
				'installing'       => esc_html__( 'Instalando', 'epicjungle' ),
				'success'          => esc_html__( 'Sucesso', 'epicjungle' ),
		...
		if ( ! empty( $import_files['widgets'] ) ) {
			$content['widgets'] = array(
				'title'            => esc_html__( 'Widgets', 'epicjungle' ),
				'description'      => esc_html__( 'Dados de widgets de amostra.', 'epicjungle' ),
				'pending'          => esc_html__( 'Pendente', 'epicjungle' ),
				'installing'       => esc_html__( 'Instalando', 'epicjungle' ),
				'success'          => esc_html__( 'Sucesso', 'epicjungle' ),
				'install_callback' => array( 'EpicJungle_Wizard_Widget_Importer', 'import' ),
				'checked'          => $this->is_possible_upgrade() ? 0 : 1,
		...
		if ( ! empty( $import_files['sliders'] ) ) {
			$content['sliders'] = array(
				'title'            => esc_html__( 'Revolution Slider', 'epicjungle' ),
				'description'      => esc_html__( 'Dados de exemplo do Revolution Slider.', 'epicjungle' ),
				'pending'          => esc_html__( 'Pendente', 'epicjungle' ),
				'installing'       => esc_html__( 'Instalando', 'epicjungle' ),
				'success'          => esc_html__( 'Sucesso', 'epicjungle' ),
		...
		if ( ! empty( $import_files['options'] ) ) {
			$content['options'] = array(
				'title'            => esc_html__( 'Opções', 'epicjungle' ),
				'description'      => esc_html__( 'Dados de opções de tema de amostra.', 'epicjungle' ),
				'pending'          => esc_html__( 'Pendente', 'epicjungle' ),
				'installing'       => esc_html__( 'Instalando', 'epicjungle' ),
				'success'          => esc_html__( 'Sucesso', 'epicjungle' ),
				'install_callback' => array( 'EpicJungle_Wizard_Customizer_Importer', 'import' ),
				'checked'          => $this->is_possible_upgrade() ? 0 : 1,
		...
		if ( ! empty( $import_files['redux'] ) ) {
			$content['redux'] = array(
				'title'            => esc_html__( 'Opções Redux', 'epicjungle' ),
				'description'      => esc_html__( 'Opções do Fremework Redux.', 'epicjungle' ),
				'pending'          => esc_html__( 'Pendente', 'epicjungle' ),
				'installing'       => esc_html__( 'Instalando', 'epicjungle' ),
				'success'          => esc_html__( 'Sucesso', 'epicjungle' ),
				'install_callback' => array( 'EpicJungle_Wizard_Redux_Importer', 'import' ),
				'checked'          => $this->is_possible_upgrade() ? 0 : 1,
		...
		if ( false !== has_action( 'epicjungle_wizard_after_all_import' ) ) {
			$content['after_import'] = array(
				'title'            => esc_html__( 'Após a configuração da importação', 'epicjungle' ),
				'description'      => esc_html__( 'Após a configuração da importação.', 'epicjungle' ),
				'pending'          => esc_html__( 'Pendente', 'epicjungle' ),
				'installing'       => esc_html__( 'Instalando', 'epicjungle' ),
				'success'          => esc_html__( 'Sucesso', 'epicjungle' ),
		...
		$content = apply_filters( 'epicjungle_wizard_get_base_content', $content, $this );
	...
	public function import_revolution_sliders( $file ) {
		...
		$this->logger->info( __( 'A importação do Slider Revolution foi executada', 'epicjungle' ) );
	...
	public function pt_importer_new_ajax_request_response_data( $data ) {
		...
		$data['message']  = esc_html__( 'Instalando', 'epicjungle' );
		$data['proceed']  = 'true';
		$data['action']   = 'epicjungle_wizard_content';
		$data['content']  = 'content';
		$data['_wpnonce'] = wp_create_nonce( 'epicjungle_wizard_nonce' );
	...
	public function after_content_import_setup() {
		// Set static homepage.
		$homepage = get_page_by_title( apply_filters( 'epicjungle_wizard_content_home_page_title', 'Home' ) );

		if ( $homepage ) {
			update_option( 'page_on_front', $homepage->ID );
			update_option( 'show_on_front', 'page' );

			$this->logger->debug( __( 'A página inicial foi definida', 'epicjungle' ), array( 'homepage_id' => $homepage ) );
		}

		// Set static blog page.
		$blogpage = get_page_by_title( apply_filters( 'epicjungle_wizard_content_blog_page_title', 'Blog' ) );

		if ( $blogpage ) {
			update_option( 'page_for_posts', $blogpage->ID );
			update_option( 'show_on_front', 'page' );

			$this->logger->debug( __( 'A página do blog foi definida', 'epicjungle' ), array( 'blog_page_id' => $blogpage ) );
		}
	}
	...
	public function before_content_import_setup() {
		...
		if ( ! empty( $hello_world ) ) {
			...
			$this->logger->debug( __( 'O status da postagem Hello world foi definido como rascunho', 'epicjungle' ) );
		}
	}

	/**
	 * Register the import files via the `epicjungle_wizard_import_files` filter.
	 */
	public function register_import_files() {
		$this->import_files = $this->validate_import_file_info( apply_filters( 'epicjungle_wizard_import_files', array() ) );
	}
	...
	public function validate_import_file_info( $import_files ) {
		...
		foreach ( $import_files as $import_file ) {
			if ( ! empty( $import_file['import_file_name'] ) ) {
				...
				$this->logger->warning( __( 'Esta importação de demonstração predefinida não tem o parâmetro name: import_file_name', 'epicjungle' ), $import_file );
			}
		}
	...
	public function set_import_file_base_name() {
		$existing_name = get_transient( 'epicjungle_wizard_import_file_base_name' );
		...
		set_transient( 'epicjungle_wizard_import_file_base_name', $this->import_file_base_name, MINUTE_IN_SECONDS );
	}
	...
	public function get_import_files_paths( $selected_import_index ) {
		...
		$import_files   = array(
			'content' => esc_html__( 'Conteúdo', 'epicjungle' ),
			'widgets' => esc_html__( 'Widgets', 'epicjungle' ),
			'options' => esc_html__( 'Opções', 'epicjungle' ),
			'redux'   => array(),
			'sliders' => esc_html__( 'Sliders', 'epicjungle' ),
		);

		$downloader = new EpicJungle_Wizard_Downloader();
	...
	/**
	 * AJAX callback for the 'epicjungle_wizard_update_selected_import_data_info' action.
	 */
	...
	public function get_import_steps_html( $import_info ) {
		...
		?>
			<?php foreach ( $import_info as $slug => $available ) : ?>
				...
				<li class="epicjungle_wizard__drawer--import-content__list-item status status--Pending" data-content="<?php echo esc_attr( $slug ); ?>">
	...
	public function import_finished() {
		delete_transient( 'epicjungle_wizard_import_file_base_name' );
