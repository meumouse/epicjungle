...
class EpicJungle_Wizard_Widget_Importer {
	/**
	...
	public static function import( $widget_import_file_path ) {
		...
		if ( is_wp_error( $results ) ) {
			EpicJungle_Wizard_Logger::get_instance()->error( $results->get_error_message() );
		...
		EpicJungle_Wizard_Logger::get_instance()->debug( $message );
	...
	private static function process_import_file( $file ) {
		...
		if ( ! file_exists( $file ) ) {
			return new \WP_Error(
				...
				__( 'Erro: o arquivo de importação do widget não foi encontrado.', 'epicjungle' )
			);
		}
		...
		if ( empty( $data ) ) {
			return new \WP_Error(
				...
				__( 'Erro: o arquivo de importação do widget não tem nenhum conteúdo nele.', 'epicjungle' )
			);
		}
	...
	private static function import_data( $data ) {
		...
		if ( empty( $data ) || ! is_object( $data ) ) {
			return new \WP_Error(
				...
				__( 'Erro: os dados de importação do widget não puderam ser lidos. Por favor, tente um arquivo diferente.', 'epicjungle' )
			);
		}

		// Hook before import.
		do_action( 'epicjungle_wizard_widget_importer_before_widgets_import', $data );
		$data = apply_filters( 'epicjungle_wizard_before_widgets_import_data', $data );
		...
		foreach ( $data as $sidebar_id => $widgets ) {
			...
			if ( isset( $wp_registered_sidebars[ $sidebar_id ] ) ) {
				...
				$sidebar_message      = __( 'A barra lateral não existe no tema (movendo o widget para Inativo)', 'epicjungle' );
			}
			...
			foreach ( $widgets as $widget_instance_id => $widget ) {
				...
				if ( ! $fail && ! isset( $available_widgets[ $id_base ] ) ) {
					...
					$widget_message      = __( 'O site não suporta widget', 'epicjungle' ); // Explain why widget not imported.
				}

				// Filter to modify settings object before conversion to array and import.
				// Leave this filter here for backwards compatibility with manipulating objects (before conversion to array below).
				// Ideally the newer wie_widget_settings_array below will be used instead of this.
				$widget = apply_filters( 'epicjungle_wizard_widget_settings', $widget ); // Object.
				...
				$widget = apply_filters( 'epicjungle_wizard_widget_settings_array', $widget );
				...
				if ( ! $fail && isset( $widget_instances[ $id_base ] ) ) {
					...
					foreach ( $single_widget_instances as $check_id => $check_widget ) {
						...
						if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
							...
							$widget_message      = __( 'O widget já existe', 'epicjungle' ); // Explain why widget not imported.
				...
				if ( ! $fail ) {
					...
					do_action( 'epicjungle_wizard_widget_importer_after_single_widget_import', $after_widget_import );

					// Success message.
					if ( $sidebar_available ) {
						$widget_message_type = 'success';
						$widget_message      = __( 'Importado', 'epicjungle' );
					}
					else {
						$widget_message_type = 'warning';
						$widget_message      = __( 'Importado para inativo', 'epicjungle' );
					}
				}

				// Result for widget instance.
				$results[ $sidebar_id ]['widgets'][ $widget_instance_id ]['name']         = isset( $available_widgets[ $id_base ]['name'] ) ? $available_widgets[ $id_base ]['name'] : $id_base; // Widget name or ID if name not available (not supported by site).
				$results[ $sidebar_id ]['widgets'][ $widget_instance_id ]['title']        = ! empty( $widget['title'] ) ? $widget['title'] : __( 'No Title', 'epicjungle' ); // Show "No Title" if widget instance is untitled.
		...
		// Hook after import.
		do_action( 'epicjungle_wizard_widget_importer_after_widgets_import', $data );

		// Return results.
		return apply_filters( 'epicjungle_wizard_widget_import_results', $results );
	}
	...
	private static function available_widgets() {
		...
		return apply_filters( 'epicjungle_wizard_available_widgets', $available_widgets );
	}
	...
	private static function unset_default_widgets() {
		$widget_areas = apply_filters( 'epicjungle_wizard_unset_default_widgets_args', false );
	...
	private static function format_results_for_log( $results ) {
		if ( empty( $results ) ) {
			esc_html_e( 'Nenhum resultado para importação de widget!', 'epicjungle' );
		}
