...
class EpicJungle_Wizard_Downloader {
	/**
	...
	public function download_file( $url, $filename ) {
		...
		if ( is_wp_error( $content ) ) {
			EpicJungle_Wizard_Logger::get_instance()->error( $content->get_error_message(), array( 'url' => $url, 'filename' => $filename ) );
		...
		EpicJungle_Wizard_Logger::get_instance()->error( __( 'O arquivo não pôde ser salvo no disco ao tentar baixá-lo', 'epicjungle' ), array( 'url' => $url, 'filename' => $filename ) );
	...
	private function get_content_from_url( $url ) {
		...
		if ( empty( $url ) ) {
			return new \WP_Error(
				'missing_url',
				__( 'URL ausente para baixar um arquivo!', 'epicjungle' )
			);
		}
		...
		$response = wp_remote_get(
			$url,
			array( 'timeout' => apply_filters( 'epicjungle_wizard_timeout_for_downloading_import_file', 20 ) )
		);
		...
		if ( is_wp_error( $response ) || 200 !== $response['response']['code'] ) {
			...
			return new \WP_Error(
				'download_error',
				sprintf(
					__( 'Ocorreu um erro ao buscar o arquivo de: %1$s%2$s%3$s!%4$sMotivo: %5$s - %6$s.', 'epicjungle' ),
					'<strong>',
					$url,
					'</strong>',
					'<br>',
	...
	public function set_download_directory_path( $download_directory_path ) {
		if ( file_exists( $download_directory_path ) ) {
			...
			$this->download_directory_path = apply_filters( 'epicjungle_wizard_upload_file_path', trailingslashit( $upload_dir['path'] ) );
		}
	}
