...
class EpicJungle_Wizard_Customizer_Importer {
	/**
	...
	public static function import( $customizer_import_file_path ) {
		...
		EpicJungle_Wizard_Logger::get_instance()->info( __( 'A importação do personalizador foi concluída com sucesso', 'epicjungle' ) );
		return true;
	}
	...
	public static function import_customizer_options( $import_file_path ) {
		...
		if ( ! file_exists( $import_file_path ) ) {
			return new \WP_Error(
				...
				sprintf(
					esc_html__( 'Erro: O arquivo de importação do personalizador está ausente! Caminho de arquivo: %s', 'epicjungle' ),
					$import_file_path
				)
			);
		}
		...
		if ( empty( $raw ) ) {
			return new \WP_Error(
				...
				esc_html__( 'Erro: O arquivo de importação do personalizador não possui nenhum conteúdo. Certifique-se de usar o arquivo de importação do personalizador correto.', 'epicjungle' )
			);
		}
		...
		if ( ! is_array( $data ) && ( ! isset( $data['template'] ) || ! isset( $data['mods'] ) ) ) {
			return new \WP_Error(
				...
				esc_html__( 'Erro: O arquivo de importação do customizador não está em um formato correto. Certifique-se de usar o arquivo de importação do personalizador correto.', 'epicjungle' )
			);
		}
		if ( $data['template'] !== $template ) {
			return new \WP_Error(
				'customizer_import_wrong_theme',
				esc_html__( 'Erro: O arquivo de importação do personalizador não é adequado para o tema atual. Você só pode importar configurações do personalizador para o mesmo tema ou um tema filho.', 'epicjungle' )
			);
		}

		// Import images.
		if ( apply_filters( 'epicjungle_wizard_customizer_import_images', true ) ) {
		...
		if ( isset( $data['options'] ) ) {
			...
			foreach ( $data['options'] as $option_key => $option_value ) {
				$option = new EpicJungle_Wizard_Customizer_Option( $wp_customize, $option_key, array(
					'default'    => '',
					'type'       => 'option',
		...
		$use_wp_customize_save_hooks = apply_filters( 'epicjungle_wizard_enable_wp_customize_save_hooks', false );
