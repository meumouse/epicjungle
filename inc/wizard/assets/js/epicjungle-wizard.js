...
var EpicJungle_Wizard = (function($){
    ...
    function window_loaded(){
    	...
    	var
    	body 		= $('.epicjungle_wizard__body'),
    	body_loading 	= $('.epicjungle_wizard__body--loading'),
    	body_exiting 	= $('.epicjungle_wizard__body--exiting'),
    	drawer_trigger 	= $('#epicjungle_wizard__drawer-trigger'),
    	drawer_opening 	= 'epicjungle_wizard__drawer--opening';
    	drawer_opened 	= 'epicjungle_wizard__drawer--open';
    	...
    	$('.epicjungle_wizard__button--proceed:not(.epicjungle_wizard__button--closer)').click(function (e) {
		    e.preventDefault();
        ...
        $(".epicjungle_wizard__button--closer").on('click', function(e){
        ...
        $(".button-next").on( "click", function(e) {
            e.preventDefault();
            var loading_button = epicjungle_wizard_loading_button(this);
            if ( ! loading_button ) {
                return false;
            }
            ...
            if( data_callback && typeof callbacks[data_callback] !== "undefined"){
				...
				$( document ).on( 'change', '.js-epicjungle_wizard-demo-import-select', function() {
					var selectedIndex  = $( this ).val();

					$( '.js-epicjungle_wizard-select-spinner' ).show();

					$.post( epicjungle_wizard_params.ajaxurl, {
						action: 'epicjungle_wizard_update_selected_import_data_info',
						wpnonce: epicjungle_wizard_params.wpnonce,
						selected_index: selectedIndex,
					}, function( response ) {
						if ( response.success ) {
							$( '.js-epicjungle_wizard-drawer-import-content' ).html( response.data );
						}
						else {
							alert( epicjungle_wizard_params.texts.something_went_wrong );
						}

						$( '.js-epicjungle_wizard-select-spinner' ).hide();
					} )
						.fail( function() {
							$( '.js-epicjungle_wizard-select-spinner' ).hide();
							alert( epicjungle_wizard_params.texts.something_went_wrong )
    ...
    function ChildTheme() {
    	var body 				= $('.epicjungle_wizard__body');
        ...
        function do_ajax() {
            jQuery.post(epicjungle_wizard_params.ajaxurl, {
                action: "epicjungle_wizard_child_theme",
                wpnonce: epicjungle_wizard_params.wpnonce,
        ...
        return {
            init: function(btn) {
                complete = function() {
                	...
                	setTimeout(function(){
				$(".epicjungle_wizard__body").addClass('js--finished');
			},1500);

                	body.removeClass( drawer_opened );

                	setTimeout(function(){
				$('.epicjungle_wizard__body').addClass('exiting');
			},3500);
...
function ActivateLicense() {
    	var body 		= $( '.epicjungle_wizard__body' );
    	var wrapper 		= $( '.epicjungle_wizard__content--license-key' );
        ...
        function ajax_callback(r) {
            ...
            if (typeof r.success !== "undefined" && r.success) {
              notice.siblings( '.error-message' ).remove();
                ...
                $( '.js-epicjungle_wizard-license-activate-button' ).removeClass( 'epicjungle_wizard__button--loading' ).data( 'done-loading', 'no' );
                notice.siblings( '.error-message' ).remove();
        ...
        function do_ajax() {
            ...
            jQuery.post(epicjungle_wizard_params.ajaxurl, {
              action: "epicjungle_wizard_activate_license",
              wpnonce: epicjungle_wizard_params.wpnonce,
...
function PluginManager(){
        ...
        function process_current(){
            if(current_item){
                ...
                if($check.is(":checked")) {
                    jQuery.post(epicjungle_wizard_params.ajaxurl, {
                        action: "epicjungle_wizard_plugins",
                        wpnonce: epicjungle_wizard_params.wpnonce,
                        slug: current_item,
                    }, ajax_callback).fail(ajax_callback);
                }else{
        ...
        function find_next(){
            ...
            var $li = $(".epicjungle_wizard__drawer--install-plugins li");
            $li.each(function(){
                var $item = $(this);
        ...
        return {
            init: function(btn){
                $(".epicjungle_wizard__drawer--install-plugins").addClass("installing");
                $(".epicjungle_wizard__drawer--install-plugins").find("input").prop("disabled", true);
                complete = function(){
    ...
    function ContentManager(){
                ...
                if(typeof response.url !== "undefined"){
                    ...
                    if(response.hash === current_item_hash){
                        ...
                        if ( typeof response.selected_index === "undefined" ) {
                            response.selected_index = $( '.js-epicjungle_wizard-demo-import-select' ).val() || 0;
                        }
            ...
            if(current_item){
                ...
                if($check.is(":checked")) {
                    jQuery.post(epicjungle_wizard_params.ajaxurl, {
                        ...
                        selected_index: $( '.js-epicjungle_wizard-demo-import-select' ).val() || 0
                    }, ajax_callback).fail(ajax_callback);
                }else{
                    $current_node.addClass("skipping");
                    setTimeout(find_next,300);
        ...
        function find_next(){
            ...
            var $items = $(".epicjungle_wizard__drawer--import-content__list-item");
            var $enabled_items = $(".epicjungle_wizard__drawer--import-content__list-item input:checked");
            $items.each(function(){
        ...
        function init_content_import_progress_bar() {
            if( ! $(".epicjungle_wizard__drawer--import-content__list-item .checkbox-content").is( ':checked' ) ) {
                return false;
            }

            jQuery.post(epicjungle_wizard_params.ajaxurl, {
                action: "epicjungle_wizard_get_total_content_import_items",
                wpnonce: epicjungle_wizard_params.wpnonce,
                selected_index: $( '.js-epicjungle_wizard-demo-import-select' ).val() || 0
            }, function( response ) {
        ...
        function update_progress_bar() {
            $('.js-epicjungle_wizard-progress-bar').css( 'width', (current_content_import_items/total_content_import_items) * 100 + '%' );

            var $percentage = valBetween( ((current_content_import_items/total_content_import_items) * 100) , 0, 99);

            $('.js-epicjungle_wizard-progress-bar-percentage').html( Math.round( $percentage ) + '%' );
        ...
        return {
            init: function(btn){
                $(".epicjungle_wizard__drawer--import-content").addClass("installing");
                $(".epicjungle_wizard__drawer--import-content").find("input").prop("disabled", true);
                complete = function(){

			$.post(epicjungle_wizard_params.ajaxurl, {
				action: "epicjungle_wizard_import_finished",
				wpnonce: epicjungle_wizard_params.wpnonce,
				selected_index: $( '.js-epicjungle_wizard-demo-import-select' ).val() || 0
			});

			setTimeout(function(){
				$('.js-epicjungle_wizard-progress-bar-percentage').html( '100%' );
			},100);
    ...
    function epicjungle_wizard_loading_button( btn ){
        ...
        $button.addClass("epicjungle_wizard__button--loading");
