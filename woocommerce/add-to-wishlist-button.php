...
<div id="yith-wcwl-add-button" data-toggle="tooltip" data-placement="bottom" title="Adicionar aos favoritos">
    <button type="button" class="btn btn-outline-primary rounded-pill btn-icon">
        <a href="<?php echo esc_url( add_query_arg( 'add_to_wishlist', $product_id, $base_url ) )?>" rel="nofollow" data-product-id="<?php echo esc_attr( $product_id ); ?>" data-product-type="<?php echo esc_attr( $product_type ); ?>" data-original-product-id="<?php echo esc_attr( $parent_product_id ); ?>" class="<?php echo esc_attr( $link_classes ); ?>">
            <?php echo wp_kses_post( $icon ); ?>
            <?php if ( ! $is_single ): ?>
            	<span class="btn-tooltip"><?php echo esc_html__('Wishlist', 'epicjungle' );?></span>
       		<?php endif; ?>
        </a>
    </button>
</div>