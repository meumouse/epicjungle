...
<div class="cart_totals <?php echo esc_attr( ( WC()->customer->has_calculated_shipping() ) ? 'calculated_shipping' : '' ); ?>">
	...
	<h2 class="h4 pb-3"><?php esc_html_e( 'Cart totals', 'epicjungle' ); ?></h2>
	...
	<div class="shop_table shop_table_responsive pb-2">
		...
		<div class="d-flex justify-content-between align-items-center font-size-md mb-2 pb-1 cart-subtotal">
			<span class="mr-2"><?php esc_html_e( 'Subtotal', 'epicjungle' ); ?>:</span>
			<span class="text-right" data-title="<?php esc_attr_e( 'Subtotal', 'epicjungle' ); ?>"><?php wc_cart_totals_subtotal_html(); ?></span>
		</div>
		...
		if ( wc_tax_enabled() && ! WC()->cart->display_prices_including_tax() ) :
			...
			if ( WC()->customer->is_customer_outside_base() && ! WC()->customer->has_calculated_shipping() ) :
				...
				$estimated_text = sprintf( ' <small>' . esc_html__( '(estimated for %s)', 'epicjungle' ) . '</small>', WC()->countries->estimated_for_prefix( $taxable_address[0] ) . WC()->countries->countries[ $taxable_address[0] ] );
			endif;
		...
		<?php elseif ( WC()->cart->needs_shipping() && 'yes' === get_option( 'woocommerce_enable_shipping_calc' ) ) : ?>
			...
			<div class="shipping border-top pt-4 mt-4">
				<h3 class="h6 mb-3 pb-1 text-center"><?php esc_html_e( 'Shipping', 'epicjungle' ); ?></h3>
				<div data-title="<?php esc_attr_e( 'Shipping', 'epicjungle' ); ?>"><?php woocommerce_shipping_calculator(); ?></div>
			</div>
		...
		<div class="order-total text-center border-top pt-4 mt-4">
			<h3 class="font-weight-normal mb-0" data-title="<?php esc_attr_e( 'Total', 'epicjungle' ); ?>"><?php wc_cart_totals_order_total_html(); ?></h3>
		</div>
